...
- **Rail global déplié par défaut (216 px, icône + libellé)** ; repli en 60 px icônes uniquement via l'icône « plier » en bas du rail, choix persisté. Entrées : Accueil, Opérations, Invitations et demandes, Recherche / Équipe, Sécurité, Aide. Pas d'entrée globale « Ma levée » ni « Data rooms ».
...
Spec complète dans **`TRANSITIONS.md`** ; prototypes exécutables `70`/`71`/`72` (72 = flow intégral, source de vérité). Règle : navigation = push droite→gauche 480 ms, action contextuelle = tiroir droit, destructif/limite = modale ; data room = drill-down en cascade.
